...
% Development path
\begin{frame}{Development is neither linear nor irreversible}
...
% Convergence chart (new)
\begin{frame}{Relative to U.S. income, Brazil is where it was in 1960}
  \datagraphic{fig_convergence.pdf}
  \begin{itemize}
    \item Brazil converged fast until 1980, diverged during the debt-crisis decade, and has fluctuated around 14--17\% since.
    \item This is the fact the whole lecture tries to explain --- not any single recession.
  \end{itemize}
...
\begin{frame}{One country, five very different economies}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{.40\textwidth}
      \begin{itemize}
        \item Population and output concentrate near the coast; the Southeast alone produces about half of GDP.
        \item The Northeast holds 27\% of the people and 14\% of the output --- the largest regional income gap.
        \item The Amazon and the Cerrado make land use, agriculture, and climate part of the growth agenda.
        \item Keep this map in mind: the binding constraint differs by place.
      \end{itemize}
  \end{columns}
  \source{IBGE, 2022 Population Census and Regional Accounts 2021; shares rounded.}
\end{frame}

% Timeline (new)
\begin{frame}{Six moments explain Brazil's starting point}
  \begin{center}
  ...
  \vspace{.2cm}
  \begin{itemize}
    \item The lecture concentrates on the period after 1994, but the achievements and scars of this longer history set the stage.
  \end{itemize}
\end{frame}
...
\begin{frame}{Development is broader than current GDP growth}
  ...
  \vspace{.35cm}
  \[
    \underbrace{\frac{Y}{N}}_{\substack{\text{income}\\\text{per person}}}
    =\underbrace{\frac{Y}{L}}_{\substack{\text{output}\\\text{per worker}}}
    \times
    \underbrace{\frac{L}{N}}_{\substack{\text{workers}\\\text{per person}}}
    \qquad
    \muted{\text{\small $Y$ = total income, $L$ = workers, $N$ = population}}
  \]
  \begin{itemize}
...
% Glossary (new)
\begin{frame}{Six terms you will hear all afternoon}
  \small
  ...
  \muted{\small Everything else --- URV, CBS/IBS, Selic --- is defined when it appears.}
\end{frame}
...
\begin{frame}{The lecture follows two questions}
  ...
  \end{columns}
  \vspace{.35cm}
  \muted{\small Backup slides contain formulas, full tables, measurement details, and references.}
\end{frame}
...
\begin{frame}{Four historical forces still shape Brazil today}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{.23\textwidth}
      ...
      \end{block}
    \column{.23\textwidth}
      \begin{exampleblock}{Natural resources}
...
\begin{frame}{Growth has been volatile since stabilization}
  ...
  \begin{itemize}
    ...
    \item This volatility explains \emph{why} the convergence line you saw never trends upward for long.
  \end{itemize}
...
\begin{frame}{The Real Plan changed the unit of account before the currency}
  ...
  \begin{beamercolorbox}[rounded=true,sep=.6em,wd=\textwidth]{exampleblock body}
    \key{Concrete example:} a store listed rice at \textbf{URV 5} for months while the price tag in cruzeiros reais changed almost daily with the URV's dollar-linked value. On July 1, 1994, ``URV 5'' simply became \textbf{R\$5}.
  \end{beamercolorbox}
  \vspace{.3cm}
  \begin{itemize}
    \item The URV coordinated prices and expectations without an overnight freeze --- an institutional solution, not just a new banknote.
  \end{itemize}
...
\begin{frame}{The policy trilemma: Brazil moved corners in 1999}
  \begin{columns}[T,onlytextwidth]
    \column{.46\textwidth}
      \begin{center}
      ...
      \begin{tikzpicture}
        ...
        \fill[WarmGold] ($(A)!0.5!(C)$) circle (3.2pt);
        \node[right=2pt,font=\scriptsize\bfseries,text=WarmGold!70!black] at ($(A)!0.5!(C)$) {1995--98};
        \fill[BrazilGreen] ($(B)!0.5!(C)$) circle (3.2pt);
        \node[below=3pt,font=\scriptsize\bfseries,text=BrazilGreen] at ($(B)!0.5!(C)$) {since 1999};
      \end{tikzpicture}
      }
      \end{center}
    \column{.51\textwidth}
      \begin{itemize}
        ...
        \item \textbf{1995--98:} a quasi-stable exchange rate with open capital markets --- so interest rates had to defend the currency.
        \item \textbf{Since 1999:} monetary policy targets inflation and the currency floats.
        \item Capital-flow measures change the constraint but do not remove it.
      \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{1994--1999: the exchange rate served as the nominal anchor}
  \centering
  ...
  \begin{itemize}
    \item \key{Why it helped:} a stable currency price disciplined tradable-goods prices and coordinated expectations after hyperinflation.
    \item \warn{Why pressure accumulated:} domestic inflation exceeded foreign inflation, the real appreciated, and high interest rates defended the corridor.
    \item External shocks (Asia 1997, Russia 1998) repeatedly tested reserve and interest-rate defenses.
  \end{itemize}
  \end{minipage}
  \source{BCB, 1999 FX Market Analysis and official FX-policy pages; dates before 1994 are a schematic summary.}
\end{frame}

\begin{frame}{Decision point: January 1999}
  \question{It is January 12, 1999. Reserves are falling fast. You run the central bank. Do you raise interest rates again, devalue within the band, impose capital controls, or float the currency?}
  \vspace{.4cm}
  \begin{center}
  ...
  \vspace{.35cm}
  \begin{itemize}
    \item None of the options was free: each allocates the adjustment to someone --- borrowers, exporters, taxpayers, or savers.
...
\begin{frame}{1999 changed who absorbs the shock}
  \begin{columns}[T,onlytextwidth]
    \column{.48\textwidth}
      \begin{block}{Before: defend the parity}
        Interest rates and reserves absorbed external shocks so the exchange-rate path could hold.
      \end{block}
      \vspace{.25cm}
      \begin{block}{After: let the price move}
        The currency absorbs part of the shock; the policy rate responds to the inflation outlook.
      \end{block}
    \column{.48\textwidth}
      \begin{exampleblock}{Adjustment since 1999}
        The market determines the exchange rate; the BCB intervenes only against disorderly conditions, not to set a level.
      \end{exampleblock}
      \vspace{.25cm}
...
\begin{frame}{A depreciation reaches inflation through several margins}
  ...
  \vspace{.3cm}
  \begin{itemize}
    \item A 2026 BCB exercise: a 10\% depreciation adds about 1.0 point to consumer inflation over 12 months --- more in crises, when expectations are unanchored, less in calm times.
    \item Monetary policy responds to the inflation forecast, not mechanically to the exchange rate.
  \end{itemize}
  \source{BCB, March 2026 Monetary Policy Report, pass-through box; BCB Working Papers 5, 181, and 473. Estimates are episode-dependent.}
\end{frame}
...
\begin{frame}{A weaker currency has ambiguous development effects}
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}p{2.6cm}XX}
    \toprule
    Who & Possible gain from depreciation & Possible cost \\
    \midrule
  ...
  \source{Full five-actor version, including firms and the public sector, in the appendix.}
\end{frame}
...
% Part I - Brazil today
% =============================================================================
\sectionframe{Part I -- Brazil today}{Institutions, inequality, production, and state capacity}
